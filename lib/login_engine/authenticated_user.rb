...
#        set_table_name LoginEngine.config(:user_table)
        ...
        #validates_presence_of :ic_number, :on => :create
        #validates_presence_of :name, :on => :create
        #validates_presence_of :email, :on => :create
        #validates_presence_of :department, :on => :create
        ...
        attr_accessor :password, :password_confirmation, :password_old
    ...
    module ClassMethods
      ...
      def authenticate(ic_number, pass)
        u = find(:first, :conditions => ["ic_number = ? AND verified = 1 AND deleted = 0", ic_number])
        return nil if u.nil?
        find(:first, :conditions => ["ic_number = ? AND salted_password = ? AND verified = 1", ic_number, AuthenticatedUser.salted_password(u.salt, AuthenticatedUser.hashed(pass))])
      end
