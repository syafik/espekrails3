...
pg_connect("host=localhost port=5432 dbname=instun user=pgsql password=genpass");
...
for($j=0 ; $j < sizeof($course_application_ids) ; $j++){ 
	$sql = "SELECT name,ic_number FROM profiles WHERE id='$course_application_ids[$j]'";
	$res = pg_exec($sql);
...
$pdf->Cell(0,5," $course_implementation_code $course_application_ids[$j] ",0,1,'R');
