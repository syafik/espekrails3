...
class CourseManagementController < ApplicationController
  ...
  def hantar_tempahan
    ...
    EspekMailer.hantar_tempahan(@course_implementation.id, params[:tempahan_sijil]["penerima"]).deliver rescue puts "Failed send email"
  ...
  def evaluation
    @course_implementation = CourseImplementation.find_by_code(params[:course_implementation_code]) if params[:course_implementation_code]
    @course_implementation = CourseImplementation.find( params[:id] || params[:course_management_id]) if (params[:id] && !params[:id].blank?) || (params[:course_management_id] && params[:course_management_id] != "")
...
logger.info "--------------------------------"
  ...
  def cetak_yuran
    ...
    pdf_render_hash = {}
    pdf_render_hash[:pdf] = "cetak_yuran.pdf"
    pdf_render_hash[:page_size] = 'A4'
